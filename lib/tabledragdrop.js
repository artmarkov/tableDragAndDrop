	...
	$.fn.tabledragdrop = function(callerSettings)
	{
		...
		function colOnMouseDown(ev)
		{
			if(!movableObject.active)
			{
				...
				var trg = $($(ev.target).parents('th')[0]);
				...
				$(document).disableSelection().css('cursor', 'move')
		...
		{
			$(document).css({cursor: 'auto'}).enableSelection().unbind('mousemove');
